...
pubDate: 2026-08-13
description: "A dozen Claude agents in one iTerm2 window, one tab each, through tmux control mode — and the tab-title bug that took two wrong fixes before I found the right one"
...
**how do you actually sit in front of a dozen agents at once?**

The answer turned out to be one iTerm2 window with a tab per agent — which took
about twenty lines of shell, and then one genuinely annoying afternoon.
...
What I wanted was one script that brings up a *category*: one tmux session, one
window per agent, each running `claude` in that agent's own directory.

Three of those exist now — one for the four agents that share a codebase, one for
the seven ops specialists, and one for the independent standalone agents. The
membership is a list at the top of each script, so enrolling a new agent is
adding one line.
...
Open that profile and I am looking at every standalone agent, one tab each, on a
machine somewhere else. Drop `TMUX_CC=1` and the identical script gives an
ordinary terminal attach.
...
Sitting a dozen agents in one window makes them look like a team in a room. They
are not.
