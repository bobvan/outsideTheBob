...
My DNS servers are not an optional convenience I could route around, and without ever thinking about it I had given them two entirely different jobs.

The first is a job only they can do.
I run split-horizon DNS: my internal DNS servers answer with internal addresses unknown to public DNS servers.
Nothing else in the world holds those answers, so everything in the home is required to ask them.

The second is a job anyone could do.
They are also the recursive resolvers for every name outside my domain — the ordinary lookups a phone makes before it can do anything at all.

Coupling those two is the mistake, and it took a blackout to show me why.
The servers that only I can provide had quietly become the servers that everything else routes through.
Losing them does not cost you your internal names. It costs you all of them.
...
My clients must use my servers, and my servers are all that stands between them and the entire Internet.
...
It has to be both, because a survivor that does only one of them still strands the clients — half an answer is no answer.
