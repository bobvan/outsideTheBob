...
[[? One factual gap in the car battery scene, and a reader will find it.

The post says everything died at fourteen minutes, access points included. So when you fed the modems and the gateway off the inverter, what were your WiFi devices associating *to*? Three possibilities I can write, but I don't want to invent one:

a) You ran an access point off the inverter too.
b) You fed an AP from the gateway's own PoE — the UCG-Fiber has a 30 W budget, which covers a U6 Lite with room to spare. This is my guess, and it's a lovely detail if true: the subsistence tier already existed inside one box.
c) You were testing over a cable, not WiFi, and "WiFi devices" is shorthand for the phones and laptops that were stranded.

Tell me which and I'll put one clause in. As written the scene has a hole in it. ?]]
