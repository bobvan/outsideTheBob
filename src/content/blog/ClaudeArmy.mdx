...
So the unit of interaction is not "ask, wait, read." It is **check in, correct,
leave again** — and tmux is what makes that cost nothing. The same sessions I
...
The third habit, and the one that turned out to matter most for the architecture.
Each agent works in its own git worktree of the same repository — separate
directories, separate checkouts, one shared history. Agents can work on different
branches of the same problem without tripping over each other's files, and
everything still converges through git.

I did not know at the time how much this decision was going to decide for me.
...
cluster. Inside that container the agents have real authority — passwordless sudo
and control of the lab resources they work on. Outside it, they have exactly what
I have handed them: specific credentials, scoped to specific jobs.
...
Those questions have good answers here. It is a container I can snapshot, roll
back, or throw away, on a cluster that can move it between nodes. That is the
trade — the agents are trusted and fast inside a boundary I can actually defend,
...
And it explained my accident. My four project agents shared a repo, so they
shared one memory store — which is why they had felt so well coordinated. Two
directories that share a git repo share their retained knowledge no matter how
separate they look.
...
become a git repository, so every agent underneath it — Proxmox, DNS, the blog,
all of them — was pooling its memories into one store and reading everybody
else's facts. I had been carefully separating agents into their own directories
and running a global hive mind the whole time.
...
project's jargon gets defined once.
...
difference is the whole ballgame and it is easy to get wrong by accident.
...
[[? Two things left, both smaller than last time.

1) The histogram is real data — 1595 prompts, every agent directory, measured from your prompt to the last transcript record before your next one. That end point is when the agent went idle rather than when you came back, so it should be the agent working. Worth a sanity check against your intuition: does a 3.2 min median with a 14 min p90 match what it feels like?

2) Still no war story from the four-session period. The post now has the stray `~/.git` and the state-storage blind spot, which helps, but one concrete "and then two agents did X" would still be the best thing you could add. ?]]
