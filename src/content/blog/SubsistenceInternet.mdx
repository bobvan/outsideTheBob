...
A client takes a list of DNS servers and treats them as interchangeable; there is no way to tell a phone to ask one server about my domain and a different one about everything else — certainly not across a home full of laptops, tablets, televisions and appliances that will never agree on how to be configured.
...
Not the ISPs, not the gateway, not the WiFi — those are the things you would list if someone asked what your Internet depends on, and that night all three of them were working.
...
**And I had never compared what the two halves cost to keep running.**
...
Keeping my DNS answers alive meant keeping a Proxmox cluster alive, which is a different order of magnitude altogether.
I had made the cheapest part of my network depend on the most expensive part, and I had never once put those two numbers beside each other.
...
<CalloutBox type="warn" title="Follow the dependency, then price it">
...
That comparison I had never made — what each half of the network costs to keep running — is not a footnote to the design. It *is* the design.
