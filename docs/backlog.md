...
| # | item |
|---|---|
| H9 | **Narrative sweep of the garden, done 2026-08-15 — keep it swept.** Three autobiographical asides removed (`limits-of-gnss-time-accuracy`, `comparing-distant-clocks` ×2). First-person *evidence* was deliberately left: "we ran it", "this is from my own lab", σ ≈ 12 mm. The test in `style-guide.md` is whether removing the first person removes information. Worth re-running the grep after any burst of writing: `took me`, `while writing this page`, `I remember`, `one afternoon`. |
