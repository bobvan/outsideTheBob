...
// The same drawing serves the site banner and the 404 page, which is the joke
// reused rather than a second joke invented: "404" sits in the box and
// "OutsideTheSite" smashes out of it. Overridable so one generator makes both;
// bare `node scripts/make-banner.mjs` still produces the banner exactly as before.
const INSIDE = process.env.BANNER_INSIDE ?? 'Think';
const WORDMARK = process.env.BANNER_WORD ?? 'OutsideTheBob';
const OUT = process.env.BANNER_OUT ?? 'public/images/outsideTheBob.svg';
const LABEL = process.env.BANNER_LABEL ?? 'Think Outside The Bob';
...
const think = outline(INSIDE, THINK.size);
const word = outline(WORDMARK, WORD.size);

// Opt-in centring for the inside-the-box text. OFF by default, because the
// shipped banner's "Think" is hand-placed at x=47 and centring it would move
// the brand mark 17px for no reason. The 404 variant turns it on, since "404"
// is much shorter and would otherwise sit adrift against the left wall.
if (process.env.BANNER_INSIDE_CENTER) {
	THINK.x = Math.round(BOX.x + (BOX.w - think.advance) / 2) + 4;
}
...
console.error(`inside:    ${JSON.stringify(INSIDE)} ${think.advance.toFixed(1)}px advance at ${THINK.size}px, x=${THINK.x}`);
console.error(`wordmark:  ${word.advance.toFixed(1)}px advance at ${WORD.size}px`);
console.error(`kerns:     ${word.kerns.join('  ') || '(none)'}`);
console.error(`inside ink: x ${(THINK.x + think.ink.x1).toFixed(1)}..${(THINK.x + think.ink.x2).toFixed(1)}`);
