...
<ZoomableImage
	...
	alt="A histogram of 1595 prompt durations across the agent fleet. Bars for zero to one, one to two, two to five and five to ten minutes are blue and hold 249, 274, 533 and 294 prompts. Bars past ten minutes are red: 132 in the ten-to-twenty bucket, 42 in twenty-to-forty, 47 in forty-to-eighty and 24 beyond eighty minutes. Summary figures below read: median 3.2 minutes, one in ten longer than 14 minutes, one in twenty longer than 30 minutes, longest 5.6 hours."
	caption="Every prompt I have given the fleet, timed from the transcripts. The median is three
		...
		quarter of an hour, one in twenty past half an hour, and the longest ran most of a working
		day."
/>
...
### And you cannot tell from the prompt

I assumed a long prompt meant a long wait. It seemed obvious — more asked for,
more work done. So I measured that too, and it is not true.

| prompt length | prompts | median | 1 in 10 longer than |
|---|---|---|---|
| 1–5 words | 85 | 3.2 min | 14 min |
| 5–15 words | 357 | 2.8 min | 14 min |
| 15–40 words | 518 | 3.1 min | 14 min |
| 40–100 words | 348 | 3.4 min | 13 min |
| 100–300 words | 163 | 3.8 min | 16 min |
| **300+ words** | 127 | **3.4 min** | **41 min** |

Look down the median column: it is **flat**. A five-word prompt and a
three-hundred-word prompt take about three minutes either way. Across all 1,598
prompts the rank correlation between length and duration is 0.11, which is
another way of writing "none."

The only real signal is in the last column, and it is about *risk* rather than
duration. My longest prompts are usually multi-part briefs — do this, then that,
and while you are in there fix the other — and while their typical case is no
slower, their tail is three times fatter. Asking for six things does not make the
median longer; it makes a bad afternoon more likely.

Which lands somewhere more useful than where I started. **I cannot look at what I
just typed and estimate when to come back.** The information simply is not in the
prompt. So the unit of interaction is not "ask, wait, read." It is **check in,
...
My default security posture is that **malicious intent will show up eventually,
even on a nominally trusted network.** I do not get to assume my own LAN stays
friendly forever. So I treat the VM running my agents as a potentially malicious
actor and ask two questions about it: what could go wrong if code in there went
rogue, and how would I recover?
...
and it carried on — and at the time I did not give it much thought beyond
laughing about it.
...
That is exactly the trade with agents, except the eleventh employee costs nothing
...
I can also now see a plausible explanation for what happened to Charlie. My best
guess is that I restarted an agent without paying attention to the working
directory and ended up with two copies of one agent — though I am honestly not
certain that is exactly what went wrong, and I no longer have the evidence to
check. What I am certain of is that
an agent could pick up somebody else's identity at all, and that is the part I
should have chased at the time.
...
**vestigial `~/.git` left over from Gas Town**. My home directory had quietly
become a git repository, so every agent underneath it that was not inside another
nested git repo — Proxmox, DNS, the blog, all of them — was pooling its memories
into one store and reading everybody else's facts. I had been carefully
separating agents into their own directories and running a global hive mind the
whole time.
...
[[? One left, and it is a check rather than a request.

I went looking for the Charlie-as-Bravo moment in the transcripts and could not find it — I searched the bravo and charlie buckets for Charlie speaking as Bravo, and for your correction, and got nothing usable. The surviving transcripts may not reach back that far. So the story is told from your memory with no quote, which I think reads fine and is certainly honester than a reconstructed one.

The thing to check: I have hung a fair amount on that anecdote. It opens as foreshadowing and pays off twice — once as the probable cause of the `~/.git` mess, once as the reason the launcher scripts exist. If your recollection of it is hazier than the post implies, tell me and I will soften the causal claim to "I never did find out." ?]]
