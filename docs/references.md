...
## ✅ Sources for B4, B7, B8, B9

**Verified 2026-08-09.** Four review items, each closed against a primary source
or our own measurement.

**B4 — the AtomiChron compute argument, withdrawn.** Refuted by the same
Septentrio power table it was built on (`Mosaic Hardware Manual` v1.3.0, §2.5):
RTK at 1 Hz draws **670 mW** where the L-band PPP row draws **760 mW**, on the
identical module. RTK *is* integer ambiguity resolution and it costs less than
the row the argument pointed at. The link-budget half stands on its own and
never needed it. The page keeps the withdrawal visible rather than quietly
deleting it.

**B7 — antenna position, two regimes.** Survey-in is meter-class, so 3–15 ns.
An offline PPP or OPUS-Static solution on 24 h of raw observations is
centimeter-class; our operational ARPs carry σ ≈ 12 mm, which is **40 ps**. The
consequence is the point: past the second row, antenna position stops being the
limiting term and the feedline takes over.

**B8 — White Rabbit and IEEE 1588.**

- White Rabbit predates 1588-2019 by roughly a decade. Its techniques were
  folded in as the **High Accuracy profile** of that edition, so
  "IEEE 1588-2019 High Accuracy profile" is both shorter and true.
- **1588-2008 already carries a `delayAsymmetry` correction.** What no edition
  of the standard supplies is a way to *measure* what to put in it, and an
  unmeasured asymmetry defaults to zero — the assumption it was meant to relax.
  White Rabbit's contribution is the measurement, not the field.

**B9 — GLONASS and Galileo.** Navipedia,
[Time References in GNSS](https://gssc.esa.int/navipedia/index.php/Time_References_in_GNSS):

> GLONASS Time (GLONASST) is generated by the GLONASS Central Synchroniser and
> the difference between the UTC(SU) and GLONASST should not exceed 1
> millisecond **plus three hours**. […] Unlike GPS, Galileo or BeiDou, GLONASS
> time scale implements leap seconds, like UTC.

And Galileo's arrangement, from the OS-SDD v1.3 (§ on the Ground Mission
Segment):

> **The Time Service Provider (TSP):** This entity supports the GCC in the
> realisation of the Galileo System Time (GST) and its alignment to the
> Coordinated Universal Time (UTC).

So GST is the one constellation timescale with no single national laboratory
behind it — the TSP works from an ensemble of European timing laboratories.
Worth the sentence, since the page grades Galileo two paragraphs later.

**Use in:** UTC → *Choosing a GNSS timescale*, *How does GNSS time relate to
UTC?*; Antennas → *What do I put in for my antenna's position?*; Transferring →
*Distributing time: the protocols*.
